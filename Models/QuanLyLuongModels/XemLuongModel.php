...
<body>
    ...
    <div class="RContent">
        <p>Tiền Lương Của Bạn Là</p>
    <?php
include_once('QuanLyLuongModel.php');

class LuongModel extends QuanLyLuongModel
{
...
?>
<form method="post">
    <input type="submit" name="submit" value="Xem Lương Nhân Viên" class="orange-button">
    <input type="button" name="Thaotac" value="Thao tác Lương" class="orange-button-2" onclick="AUDLUONG()">
...
class LuongNVModel extends QuanLyLuongModel
{
    public static function withDifferentHost($host)
    {
        $instance = new self();
    ...
    public function displayLuongNV()
    {
      
        $this->open_db();

        $sql = "SELECT MaNV,Luong,DonVi,NgayCapNhat FROM Luong";
        $result = $this->condb->query($sql);
        if ($result->num_rows > 0) {
            echo "<br>";
            echo "<br>";
...
<script>
        ...
        function AUDLUONG (){
            window.location.href = "AUDLuongModel.php";
        }
    </script>
